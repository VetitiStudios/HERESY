...
@export var FRICTION_DELAY: float = 0.1  # Delay before friction applies
...
@export var WALL_JUMP_FORCE: float = 10.0  # Added a wall jump force to push the player away from the wall

# Timer to track how long player has been grounded
var grounded_time: float = 0.0
var air_max_speed: float = MAX_SPEED  # Store max speed when in air

func _ready():
	Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)
	
	# Safely get the UI node
...
func _physics_process(delta):
	...
	# Air Deceleration: Apply delay before friction kicks in
	if not GROUNDED:
		var horizontal_velocity = Vector3(velocity.x, 0, velocity.z)
		var speed = horizontal_velocity.length()

		if IS_MOVING and speed < air_max_speed:
			# Apply in-air acceleration if below max speed
		...
		# No friction applied while in the air
		if grounded_time > 0:
			grounded_time -= delta  # Reset grounded timer

	else:
		# Ground movement: Apply friction only after delay and if above max speed
		...
		# Update grounded time if on the ground
		if GROUNDED:
			grounded_time += delta
	...
	# Jumping (add bhop mechanics)
	if Input.is_action_just_pressed("jump"):
		if is_on_wall_only() and REMAINING_WALL_JUMPS > 0:
			var wall_normal = get_wall_collision_normal()
			# Apply wall jump force to move the player away from the wall
			velocity.x = wall_normal.x * WALL_JUMP_FORCE
			velocity.z = wall_normal.z * WALL_JUMP_FORCE
			velocity.y = JUMP_HEIGHT * 2.5  # Add vertical component for the jump
			REMAINING_WALL_JUMPS -= 1
			IS_JUMPING = true
			grounded_time = 0.0  # Reset the grounded timer when jumping
		elif GROUNDED:
